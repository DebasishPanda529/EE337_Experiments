...
void spi_init(void)
{
  	/*
			Configure SPCON=0x__ is not a bit addressable sfr
																												                                    SPEN = 1 to enable SPI module
			(1) Configure MSTR to set uC in Master mode                                           MSTR = 1
			(2) Configure SSDIS to disable SSBAR thus 
					P1.1(SSBAR) will be available as standard I/O pin                                 SSDIS = 1	             	 	 
			(3) Configure clock polarity s.t. sck from uC set to 1 in idle state                  CPOL = 1
					Refer to SPCON register table no 19-3 from AT89C5131 datasheet                    
			(4) Configure clock phase i.e. CPHA: Refer SPI timing                                 CPHA = 0
			...
			(5) Fclk/16 & Fclk Periph=12MHz ,thus BAUD RATE=750KHz                                SPR2 SPR1 SPR0 = 100
